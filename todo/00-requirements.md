...
Gradle Kotlin DSL, Spring Data JPA, Flyway와 `cowork-project`를 참고한 도메인별 패키지 구조를 채택했다. JDK·Kotlin·Spring Boot·Gradle·DB·Redis의 적용 버전과 구현 범위는 [서버 기반 문서](01-foundation.md)에 기록했다. DataGSM SDK 1.6.0의 PKCE·JSON·userinfo 계약을 모의 HTTP로 검증하고 D-01·D-04를 채택했다. 실제 학교 클라이언트 등록과 나머지 정책 결정은 남아 있어 `FND-01` 전체를 완료 처리하지 않는다.
...
| --- | --- | --- | --- |
| D-01 | 재학생 외 교사·졸업생·자퇴생 허용 여부 | **채택:** ACTIVE 재학생만. STUDENT, 유효 학년, 재학생 역할, isLeaveSchool=false를 검사. 학교 역할을 LCG 운영자 권한으로 승격하지 않음 | 사용자 확인 완료, 인증 구현 반영 |
...
| D-04 | 로그인 세션 방식·웹 배포 도메인 | **채택:** 같은 사이트의 웹/API + Redis 서버 세션, HttpOnly/Secure/SameSite=Lax 쿠키, 변경 요청 CSRF 방어. 30분 미사용·8시간 절대 만료 | 사용자 확인 완료, 인증 구현 반영 |
