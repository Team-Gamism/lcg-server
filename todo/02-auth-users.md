...
- PKCE 코드 교환 JSON 필드: `grant_type=authorization_code`, `code`, `client_id`, `redirect_uri`, `code_verifier`. 기본 OAuth 라이브러리의 form 전송 방식과 다르므로 JSON 교환을 별도 구현/검증한다. [교환 요청](https://docs.datagsm.kr/oauth/http/token-exchange)
...
sequenceDiagram
    ...
    L->>D: JSON token 교환 + verifier
    D-->>L: 학교 토큰
    L->>D: Bearer로 userinfo 조회
...
- [ ] **AUTH-01 | P0 | OAuth 클라이언트·설정 준비** — 선행: `FND-01~02`.
  - 구현: [클라이언트 관리](https://www.datagsm.kr/clients)에서 환경별 client 등록, callback URI·최소 scope, 설정 검증, secret 사용 여부를 정한다. 비밀값은 환경 설정으로 주입한다.
  - 완료 기준: 개발/운영 callback이 정확히 등록되고 설정 문서에는 변수명과 예시만 남는다. 외부 자격 증명 없이도 모의 서버로 다음 구현을 진행할 수 있다.
...
- [ ] **AUTH-03 | P0 | 콜백·토큰 교환·학교 회원 upsert** — 선행: `AUTH-02`, `FND-03`.
  - 구현: state/상관 쿠키 확인 및 원자적 일회성 소비 → JSON 코드 교환 → userinfo → 자격 검사 → User/SchoolIdentity 생성·갱신을 수행한다. 이메일 변경에도 최상위 provider ID로 같은 회원을 찾는다.
...
- [ ] **AUTH-04 | P0 | LCG 세션·로그아웃·권한** — 선행: `AUTH-03`.
  - 구현: 로그인 시 세션 ID 교체, HttpOnly/Secure/SameSite 쿠키, CSRF 방어, 세션 만료, `GET /me`, 현재/전체 세션 종료, 정지·탈퇴 시 즉시 무효화를 구현한다. LCG 운영자 부여는 별도 통제한다.
