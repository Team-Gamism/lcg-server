...
작성일: 2026-09-21. 사용자 제공 서비스 기획서 1~25절과 공식 외부 API 문서를 분석했다. Kotlin + Spring Boot 서버 기반, 로컬 PostgreSQL·Redis, 공통 API와 테스트·CI 구성을 추가했다. 이 폴더는 **구현 계획과 진행 기록**이다. 체크되지 않은 항목에는 부분 구현도 포함되며, 완료 기준까지 검증한 항목만 체크한다.

2026-09-21 기술 선택을 반영해 **Kotlin + Spring Boot + PostgreSQL + Redis, Modular Monolith** 구성을 기준으로 한다. 학교 OAuth는 DataGSM 공식 Java/Kotlin SDK 1.6.0을 사용하고 모의 HTTP 서버로 계약을 검증했다. 실시간 통신 방식은 P2의 `RT-01`에서 결정하고 MVP에는 소켓 도입을 요구하지 않는다. [채택한 기술 방향](00-requirements.md#채택한-기술-방향)과 구현 현황에 기록된 내용을 제외한 API 경로, 엔티티, 운영 수치와 정책 기본안은 설계 제안이다. 프론트엔드 구현은 범위에서 제외하고 서버가 제공할 계약만 정리한다.

2026-09-22 현재 `FND-02`, `AUTH-02~05`, `USER-01`의 서버 구현·자동 검증을 완료했다. `FND-01`, `FND-03~05`, `AUTH-01`, `OPS-01`은 부분 완료다. D-01 재학생 제한과 D-04 Redis 세션을 채택했으며, 테스트 55개와 JAR 빌드가 통과했다. 자세한 범위는 [서버 기반 현황](01-foundation.md#구현-현황-2026-09-21)과 [인증 현황](02-auth-users.md#구현-현황-2026-09-22)에 기록했다. 다음은 실제 DataGSM 클라이언트 등록·브라우저 로그인 검증과 `USER-02` 공개 설정 구현이다. Riot 연동과 서비스 기능은 아직 구현하지 않았다.
