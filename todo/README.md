...
2026-09-21 기술 선택을 반영해 **Kotlin + Spring Boot + PostgreSQL + Redis, Modular Monolith** 구성을 기준으로 한다. 학교 OAuth는 DataGSM 공식 Java/Kotlin SDK를 사용할 예정이며, 채택 버전과 실제 계약의 호환성은 구현 전에 검증한다. 실시간 통신 방식은 P2의 `RT-01`에서 결정하고 MVP에는 소켓 도입을 요구하지 않는다. [채택한 기술 방향](00-requirements.md#채택한-기술-방향)과 아래 서버 기반 구현 현황에 기록된 내용을 제외한 API 경로, 엔티티, 운영 수치와 정책 기본안은 설계 제안이다. 프론트엔드 구현은 범위에서 제외하고 서버가 제공할 계약만 정리한다.

현재는 `FND-02`를 완료했고, `FND-01`, `FND-03~05`, `OPS-01`은 부분 구현 상태다. API 계약 16개와 DB·Redis 통합 6개가 통과했고 JAR 빌드를 검증했다. 채택한 버전·구조·API 계약과 남은 내용은 [서버 기반 구현 현황](01-foundation.md#구현-현황-2026-09-21)에 기록한다. 다음 작업은 `FND-01`의 인증 관련 정책과 `AUTH-01`의 DataGSM SDK 계약을 확정하는 것이다. 실제 학교 로그인·Riot 연동·서비스 기능은 아직 구현하지 않았다.
